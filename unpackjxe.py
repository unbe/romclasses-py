...
J9ROMImageHeader = Struct("J9ROMImageHeader",
	...
	Pointer(lambda ctx: ctx.toc_pointer, Array(lambda ctx: ctx.class_count, 
		Struct("J9ROMClassTOCEntry",
			Relative("class_name_pointer"),
			Relative("class_pointer"),
			StringPointer(lambda ctx: ctx.class_name_pointer, "class_name"),
			MaybePointer(lambda ctx: ctx.class_pointer, Struct("J9ROMClass",
			    ULInt32("rom_size"),
			    ULInt32("single_scalar_static_count"),
			    Relative("class_name_pointer"),
			    Relative("superclass_name_pointer"),
			    ULInt32("modifiers"),
				...
				MaybePointer(lambda ctx: ctx.class_name_pointer, PascalString("class_name", length_field=ULInt16("length"))),
				MaybePointer(lambda ctx: ctx.superclass_name_pointer, PascalString("superclass_name", length_field=ULInt16("length"))),
				MaybePointer(lambda ctx: ctx.interfaces_pointer, Array(lambda ctx: ctx.interface_count, StringRef("interface"))),
			)),
		)
	)),
)	
